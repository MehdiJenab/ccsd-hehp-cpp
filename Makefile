...
regression: build  ## Run bit-exact MPI regression
	python3 src/apps/scripts/run_mpi_regression.py --executable build/debug/bin/ccsd_code
...
bench:  ## Run canonical bench matrix and refresh report
	bash src/apps/scripts/run_bench.sh
...
bench-pgo:  ## Two-stage PGO build + bench
	...
	mpirun --oversubscribe -np 4 build/release-fast-instrument/bin/ccsd_bench --batch 200 --warmup 20
	cmake --preset release-fast-pgo
	cmake --build --preset release-fast-pgo
	python3 src/apps/scripts/run_mpi_regression.py \
	    --executable build/release-fast-pgo/bin/ccsd_code --tolerance 1e-9
